...
void Forge::SoftmaxGradsCPU::compute_grads(const Tensor& logits, const Tensor& activations) const{
    ...
    DISPATCH_ALL_TYPES(logits.dtype(), Device::CPU, [&] {
        auto logits_grads_map {logits.gradients().as_eigen<grads_t>()};
        auto act_map_f {activations.as_eigen<grads_t>()};
        auto act_grads_map {activations.gradients().as_eigen<grads_t>()};

        auto act_mul_act_grads {act_grads_map*act_map_f};
        Eigen::array<Eigen::Index, 1> reduce_axis {3};
        auto dims {act_map_f.dimensions()};
